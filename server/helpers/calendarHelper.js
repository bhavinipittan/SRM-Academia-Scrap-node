...
class CalendarFetcher {
  ...
  generateFallbackHTML() {
    ...
    let html = `<table><tr><th>${monthName} '${year
      .toString()
      .slice(2)}</th><th></th><th></th><th></th><th></th></tr>`;
  ...
  generateFallbackCalendarResponse() {
    ...
    const todayObject = monthEntry.days.find(
      (day) => parseInt(day.date) === todayDate
    );
    ...
    if (todayDate < daysInMonth) {
      tomorrowObject = monthEntry.days.find(
        (day) => parseInt(day.date) === todayDate + 1
      );
    }

    if (todayDate + 1 < daysInMonth) {
      dayAfterTomorrowObject = monthEntry.days.find(
        (day) => parseInt(day.date) === todayDate + 2
      );
    }
  ...
  parseCalendar(html) {
    try {
      ...
      if (monthEntry && monthEntry.days.length > 0) {
        ...
        if (!today) {
          console.log(
            `Today (${todayDate}) not found in calendar, using first available day`
          );
        ...
        const todayIndex = monthEntry.days.findIndex(
          (day) => parseInt(day.date, 10) === todayDate
        );
        if (todayIndex !== -1) {
          if (todayIndex + 1 < monthEntry.days.length) {
            ...
            if (todayIndex + 2 < monthEntry.days.length) {
              ...
              console.log(
                `Set day after tomorrow's day to: ${dayAfterTomorrow.day}`
              );
            }
          }
        }
  ...
  validateAndFixDayName(dayObj, monthStr) {
    try {
      ...
      if (dayObj.day !== correctDayName) {
        console.log(
          `Fixing day name for ${monthName} ${dayObj.date}: ${dayObj.day} -> ${correctDayName}`
        );
