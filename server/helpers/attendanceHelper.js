...
class AcademicsFetch {
  ...
  scrapeAttendance(html) {
    try {
      ...
      // Process HTML to get the attendance table
      let processedHtml = html.replace(/<td  bgcolor='#E6E6FA' style='text-align:center'> - <\/td>/g, '');
      const parts = processedHtml.split(`<table style="font-size :16px;" border="1" align="center" cellpadding="1" cellspacing="1" bgcolor="#FAFAD2">`);
      ...
      $('td[bgcolor="#E6E6FA"]').each((index, element) => {
        ...
        // Check if this is a course code (starts with a number and length > 10 or contains "regular")
        if ((courseCode.length > 10 && /^\d/.test(courseCode)) || courseCode.toLowerCase().includes('regular')) {
